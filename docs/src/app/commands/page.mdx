...
Use `--human` with `click`, `drag`, or `mouse move` for seeded, curved movement that starts fast and slows into the target. Cubic ease-out covers 87.5% of forward distance in the first half of the planned duration; the bend decelerates with it to avoid a final sideways hook. Human clicks retain a short settle before pressing. Target size does not affect timing, and ordinary smooth mode retains symmetric slow–fast–slow easing. For `mouse move`, `--duration` sets the target total milliseconds including browser response time, `--steps` sets the number of events, and `--seed` makes the path reproducible. A slow browser can still extend the requested duration.
...
agent-browser record start <path> --cursor-icon ./cursor.svg --cursor-scale 2 --cursor-hotspot 4,3 # Large SVG pointer with a precise tip
